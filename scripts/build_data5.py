# 국토종합진단지수 자료 적재 (v5) — 2026-08-27 5차 취합본
#
# v4에서 달라진 점
#   · 연도별 메타데이터를 살린다. 같은 지표라도 연도에 따라 정의·산식·출처가
#     다르면(예: 지역경제활력도의 생활업종 사업자수·월매출액) 그 차이를
#     지표의 per[연도]에 담아 화면이 연도에 맞는 설명을 보여 주게 한다.
#   · 지역안전(S9)은 군위군 2021·2022년 결측을 수정한 파일을 쓴다.
#   · 친환경성 방향은 ▲▲▼▲ 유지 — V0831 지표체계 표에는 1·2등급 면적비율이
#     ▼로 적혀 있으나 이는 옛 '등급 평균' 정의의 흔적으로 보고 확인받아 유지.
#
# 실행: python3 scripts/build_data5.py
...
def read_sector(sec, order):
    ...
    for c in cols:
        ...
        label0 = md.get('label') or ikey
        # 연도 한 칸의 설명 묶음 — S4처럼 정의가 산식 칸에 적힌 파일은 자리를 바꾼다
        desc, formula = md.get('desc', ''), md.get('formula', '')
        if desc == label0 and formula:
            desc, formula = formula, ''
        fs = dict(desc=desc, formula=formula, unit=md.get('unit', ''),
        ...
        if ikey not in inds:
            ...
            inds[ikey] = dict(
                id=ikey, sector=sec['key'], no=no, label=label0, dir=d,
                unit=fs['unit'], desc=fs['desc'], formula=fs['formula'],
                source=fs['source'], note=fs['note'],
                years=[], cols={},
            )
        else:
            # 이 연도의 설명이 기준(첫 연도)과 다르면 per[연도]에 차이만 담는다
            base = inds[ikey]
            diff = {k: v for k, v in fs.items() if v and v != base.get(k, '')}
            if diff:
                inds[ikey].setdefault('per', {})[str(year)] = diff
...
def main():
    ...
    payload = dict(
        meta=dict(
            ...
            source='국토종합진단지수 5차 취합 (2026-08-27) · 9개 부문 (S7 생활인프라 준비중)',
